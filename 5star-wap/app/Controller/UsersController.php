...
class UsersController extends AppController {
	...
	public $components =array('Mobile','Captcha');
   ...
   public function login() {
		if ($this->Session->read('Auth.User.Id')) {
			$this->redirect(array('controller'=>'index','action' => 'profile'));
		}
	 ...
	 public function register() {
		...
		if ($this->request->is ( 'post' )) {
			...
			if ($this->request->data ['password'] != $this->request->data ['passwordc']) {
				$error ['password'] = 'Mật khẩu và xác nhận mật khẩu không trùng nhau';
			}
			if (empty ( $this->request->data ['username'] )) {
				$error ['username'] = ' Không được để trống Tên Tài Khoản';
			}
			if (strlen ( $this->request->data ['username'] ) > 50) {
				$error ['username'] = 'Độ dài tên tài khoản quá dài ';
			}
			...
			if(!isset($this->request->data['dieukhoan'])){
				$error['dieukhoan'] = 'Bạn chưa đồng ý với các điều khoản của chúng tôi';
			} 
			if (! empty ( $error )) {
				$this->set ( 'users', $this->request->data );
				$this->set('error',$error);
			} else {
				unset($this->request->data['dieukhoan']);
				try {
				$user = $this->GameApi->resource ( 'User' )->add ($this->request->data);
					}
					catch (Exception $e){
					$this->Session->setFlash($user['name']);
					return;
				...
				if (isset ( $user ['user'] )) {
					$this->Session->setFlash ( __ ( 'Đăng ký thành công mới bạn đăng nhập vào hệ thống' ) );
					...
					if (isset ( $user ['name'] )) {
						...
						$this->Session->setFlash ( __ ( 'Không thể đăng nhập ,xin hãy thử lại ' ) );
					}
				}
			}
		} */
	...
	public function payment() {
		...
		if (!$this->Session->check('Auth.User.Id')) {			
			die(json_encode(array('status' => false, 'message' => 'Please login first before process payment!')));
		} else {
			$objData = json_decode(file_get_contents("php://input"),true);
			//$this->log($objData);
			if (isset($objData[0]['coin']) && $objData[0]['coin']) {
			  $result =	$this->GameApi->resource('User')->request('/payment/',array('method' => 'POST','data' => array('coin' => $objData[0]['coin'],'currency' => 'Wcoin')));
			  if (isset($result['user'])) {
				  die(json_encode(array('status' => true,'message' => $result['user']['User']['coin'])));
			  } else {
				  die(json_encode(array('status' => false, 'message' => $result['name'])));
			  }
			} else {
				die(json_encode(array('status' => false, 'message' => 'Invalid coin number')));
			}
		}   */
		
	}
	public function partner() {
		/* $networks = $this->GameApi->resource('Network')->query(array('type' => 'partner'));        
		$this->set('networks',$networks['networks']); */
	}
