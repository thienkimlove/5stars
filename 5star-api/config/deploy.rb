...
set :repository,  "file:///opt/repositories/svn/repos/trunk/5star-api"
...
task :set_repository do
	if exists?(:tag) and tag != ""
		set :repository, "file:///opt/repositories/svn/repos/tags/#{tag}/5star-api"
	else
		set :repository, "file:///opt/repositories/svn/repos/trunk/5star-api"    
	end
...
task :production do
	...
	set :password, "tieungao.pro.api@1234@gdata"
	set :deploy_to, "/home/production/domains/#{subname}.#{application}/"
	set :copy_remote_dir, "/home/production/deploy/tmpupload"
	set :copy_dir, "/usr/local/jenkins/stores"
	set :config_file_suffix, "production"
	server "118.69.171.36", :app, :db, :primary => true
	#after "deploy:finalize_update", "deploy:cron:payment"
end
...
task :staging do
	...
	set :user, "stage1"
	set :group, "stage1"    
	set :password, "tieungao.stage.api"
	set :deploy_to, "/home/stage1/domains/#{subname}.#{application}/" 
	set :copy_remote_dir, "/home/stage1/deploy/tmpupload"
	set :copy_dir, "/usr/local/jenkins/stores"
...
namespace :deploy do
	...
	namespace :cron do
		...
		desc "run the payment cron"
		task :payment, :roles => :app do
			run "chmod 755 #{current_release}/app/Console/cake"
			run "#{current_release}/app/Console/cake CronPayment cron"  
		end
	end
